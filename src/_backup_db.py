...
if not len(sys.argv) in [2, 3] or not os.access(sys.argv[1], os.W_OK):
    ...
    print(
        "(DIRECTORY must be writeable, RSYNC-TO is optional location to RSYNC the file to)"
    )
    sys.exit(1)
...
if hasattr(ctb.conf.misc.backup, "encryptionpassphrase") and not find_executable("gpg"):
    print(
        "encryptionpassphrase is specified but gpg executable not found, please install gpg"
    )
    sys.exit(1)

filename = "%s/%s_%s.sql.gz" % (
    sys.argv[1],
    ctb.conf.db.auth.dbname,
    datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S"),
)

print("Backing up to %s..." % filename)
os.popen(
    "mysqldump -u %s -p%s -h %s -e --opt -c %s | gzip --best -c >%s"
    % (
        ctb.conf.db.auth.user,
        ctb.conf.db.auth.password,
        ctb.conf.db.auth.host,
        ctb.conf.db.auth.dbname,
        filename,
    )
)
...
try:
    print("Encrypting...")
    os.popen(
        "gpg --batch --passphrase '%s' -c %s"
        % (ctb.conf.misc.backup.encryptionpassphrase, filename)
    )
